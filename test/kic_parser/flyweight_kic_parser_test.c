...
inline static void kic_version_test() {
  ...
  for (const char *const *head = &kic_correct_version[0];
    ...
    const unsigned char expected_available = check_kic_compatibility(*head);
    assert(expected_available == KIC_CONPATIBLE);
  }
  ...
  for (const char *const *head = &kic_incorrect_version[0];
    ...
    const unsigned char expected_unavailable = check_kic_compatibility(*head);
    assert(expected_unavailable == KIC_INCONPATIBLE);
  }
}
